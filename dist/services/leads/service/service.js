...
import { FIELD_WEIGHTS, getMonthlyName, createConversionRatesMap, getConversionRateFromMap, calculateLeadScore, getMonthIndex, isEmptyValue } from "../utils/leads.util.js";
...
export class LeadService {
     ...
     */
    async updateConversionRatesAndLeadScoresForClient(clientId) {
        const errors = [];
        try {
    ...
    async getLeadAnalytics(clientId, timeFilter = 'all') {
    ...
    async getPerformanceTables(clientId, commonTimeFilter = 'all', adSetPage = 1, adNamePage = 1, adSetItemsPerPage = 15, adNameItemsPerPage = 10, sortOptions) {
        // Build time filter query
        const query = { clientId };
     ...
     */
    async processLeadAnalytics(leads) {
     ...
     */
    async processZipAnalysis(estimateSetLeads, estimateSetCount) {
    ...
    async processServiceAnalysis(estimateSetLeads, estimateSetCount) {
    ...
    async processAdSetAnalysis(leads) {
    ...
    async processAdNameAnalysis(leads) {
    ...
    async processLeadDateAnalysis(estimateSetLeads, estimateSetCount) {
    ...
    async processUnqualifiedReasonsAnalysis(leads, unqualifiedCount) {
    ...
    async getLeadsPaginated(clientId, startDate, endDate, pagination = { page: 1, limit: 50, sortBy: 'date', sortOrder: 'desc' }, filters = {}) {
        ...
        if (startDate || endDate) {
            query.leadDate = {};
            if (startDate)
                query.leadDate.$gte = startDate; // ⚠️ if you change schema to Date, wrap with new Date(startDate)
            if (endDate)
        ...
        if (filters.adSetName)
            query.adSetName = { $regex: filters.adSetName, $options: 'i' };
        if (filters.adName)
            query.adName = { $regex: filters.adName, $options: 'i' };
        if (filters.status)
     ...
     */
    async calculateAndStoreMissingLeadScores(leads, clientId) {
        // Find leads without lead scores
        const leadsWithoutScores = leads.filter(lead => typeof lead.leadScore === 'undefined' || lead.leadScore === null);
        ...
        try {
            ...
            if (conversionRates.length === 0) {
                ...
                leads.forEach(lead => {
                    if (leadsWithoutScores.some(l => l._id.toString() === lead._id.toString())) {
                        lead.leadScore = 0;
                    }
                });
                return leads;
            ...
            leads.forEach(lead => {
                const leadWithoutScore = leadsWithoutScores.find(l => l._id.toString() === lead._id.toString());
                if (leadWithoutScore) {
    ...
    async recalculateAllLeadScores(clientId) {
        ...
        try {
            ...
            if (conversionRates.length === 0) {
                ...
                await LeadModel.updateMany({ clientId }, { $set: { leadScore: 0 } });
                return {
    ...
    async getLeads(clientId, startDate, endDate) {
        const query = {};
        if (clientId)
            query.clientId = clientId;
        if (startDate && endDate)
        ...
        if (clientId) {
            return await this.calculateAndStoreMissingLeadScores(leads, clientId);
        }
        return leads;
    }
    ...
    getUniqueFieldValues(leads) {
        ...
        const zipSet = new Set(); // NEW: ZIP code set
     ...
     */
    async updateConversionRatesWithWeeklyData(clientId, weeklyLeads) {
        ...
        for (const newData of newWeeklyData) {
            ...
            const existingRate = existingRates.find(rate => rate.keyField === newData.keyField && rate.keyName === newData.keyName);
            let updatedRate;
            if (existingRate) {
    ...
    async bulkCreateLeads(payloads, uniquenessByPhoneEmail = false) {
        ...
        const documents = []; // Return empty array to avoid expensive query
        return {
            documents,
            stats: {
                total,
    ...
    async processCompleteSheet(sheetUrl, clientId, uniquenessByPhoneEmail = false) {
